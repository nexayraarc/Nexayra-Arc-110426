...
export async function POST(req: NextRequest) {
  try {
    ...
    // Auto-increment via transaction
    ...
    return NextResponse.json({
      ok: true,
      lpo: { ...lpoData, createdAt: new Date().toISOString() },
    });
  } catch (err: any) {
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!nxrNo || !approvedBy) {
      return NextResponse.json({ ok: false, message: "Missing nxrNo or approvedBy" }, { status: 400 });
    }

    await adminDb.collection("lpos").doc(`LPO-${nxrNo}`).update({
      approved: true,
      approvedBy,
