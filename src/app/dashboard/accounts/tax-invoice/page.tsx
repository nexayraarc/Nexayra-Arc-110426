    ...
    <div>
      <div className="flex gap-1 mb-4 border-b border-navy-100">
        {tabs.map(t => {
          ...
          return <Link key={t.href} href={t.href} className={`px-3 py-2 text-xs font-semibold border-b-2 ${active ? "border-gold text-navy" : "border-transparent text-navy-400 hover:text-navy"}`}>{t.label}</Link>;
        })}
      </div>
      <CreateTaxInvoice/>
    </div>
