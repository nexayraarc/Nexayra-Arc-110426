        ...
        {modules.map((mod, i) => (
          <Link key={mod.href} href={mod.href}
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-navy font-bold text-base">{mod.label}</h3>
              <p className="text-navy-400 text-sm">{mod.desc}</p>
            </div>
            <ArrowRight size={18} className="text-navy-300 group-hover:text-navy group-hover:translate-x-1 transition-all duration-300 shrink-0" />
          </Link>
        ))}
      </div>
    </div>
