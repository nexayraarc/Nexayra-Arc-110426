...
import { FileText, FileCheck, Receipt, ArrowRight } from "lucide-react";
    ...
    async function fetchCounts() {
      try {
        const [lpoRes, qtnRes, rcRes] = await Promise.all([
          apiCall<any>("/api/lpo"),
          apiCall<any>("/api/quotation"),
          apiCall<any>("/api/receiver-copy"),
        ]);
        setCounts({
          lpos: lpoRes.lpos?.length || 0,
          quotations: qtnRes.quotations?.length || 0,
          receiverCopies: rcRes.receiverCopies?.length || 0,
        });
      } catch (err) {
  ...
  return (
    <div>
          ...
          <div className="flex gap-3 mt-6">
            {modules.map((mod) => (
              <Link key={mod.href} href={mod.href}
                className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm font-semibold transition-all backdrop-blur-sm">
                {mod.label}
              </Link>
            ))}
          </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-8">
        {/* Pie chart card */}
        <div className="bg-white rounded-2xl border border-navy-100 p-6 shadow-sm animate-fade-in-up delay-1">
          <h2 className="font-display text-lg font-bold text-navy mb-1">Documents Overview</h2>
          <p className="text-navy-400 text-sm mb-4">Total: {loading ? "…" : total} documents created</p>

          {loading ? (
            <div className="h-64 flex items-center justify-center">
              <div className="w-6 h-6 border-[3px] border-navy border-t-transparent rounded-full animate-spin" />
            </div>
          ) : total === 0 ? (
            <div className="h-64 flex items-center justify-center text-navy-300 text-sm">
              No documents yet. Create your first one!
            </div>
          ) : (
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie data={pieData} cx="50%" cy="50%" innerRadius={65} outerRadius={105} paddingAngle={4} dataKey="value" stroke="none">
                  {pieData.map((_, i) => (
                    <Cell key={i} fill={COLORS[i]} />
                  ))}
                </Pie>
                <Tooltip
                  contentStyle={{ background: "#fff", border: "1px solid #e8eaf5", borderRadius: "12px", fontSize: "13px", boxShadow: "0 4px 12px rgba(0,0,0,0.08)" }}
                />
        ...
        <div className="grid gap-4">
          {modules.map((mod, i) => (
            <Link key={mod.href} href={mod.href}
              className={`animate-fade-in-up delay-${i + 2} group flex items-center gap-5 p-5 rounded-2xl bg-white border border-navy-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300`}>
              <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${mod.gradient} flex items-center justify-center shrink-0 shadow-lg group-hover:scale-105 transition-transform`}>
              ...
              <ArrowRight size={20} className="text-navy-300 group-hover:text-navy group-hover:translate-x-1 transition-all" />
            </Link>
          ))}
        </div>
      </div>
