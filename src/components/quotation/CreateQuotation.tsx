...
const DEFAULT_EXCLUSIONS = [
  "Any materials required for the installation, including but not limited to AC units, ducting materials, communication cables, refrigerant, rubber pads. and other accessories, are not included in this quotation. All materials shall be provided by the contractor.",
  "Any kind of civil works such as foundations or steel structural support for indoor units, necessary openings for the passage of the piping and closing the openings after erection, external openings, false ceiling cutting & opening etc.",
  "All electric items as per local regulations distribution board, control panel, isolator, ELCB, P.F correction capacitors, U.V relay etc., power wiring/Conduits, up to the junction box of air conditioning units.",
  "Cranage, scaffolding, and man lift required for the installation works.",
  "Generator connection & shifting.",
  "Power and water connections required for installation and erection works.",
  "Third party testing & commissioning.",
  "Electrical & power connection.",
  "Cladding / trucking works.",
];
...
export default function CreateQuotation() {
  ...
  const [items, setItems] = useState<QuotationItem[]>([{srNo:"1",description:"",unit:"",amount:""}]);
  const [attachments, setAttachments] = useState<File[]>([]);
  const [isWorking, setIsWorking] = useState(false);
  const [message, setMessage] = useState<{text:string;type:"success"|"error"}|null>(null);
  const [validityMode, setValidityMode] = useState(DEFAULT_VALIDITY_DAYS);
  ...
  useEffect(() => {
    ...
    try {
      ...
      setTo(d.to??""); setAttn(d.attn??""); setProject(d.project??""); setServiceTitle(d.serviceTitle??"");
      if (Array.isArray(d.inclusionItems) && d.inclusionItems.length > 0) setInclusionItems(d.inclusionItems);
      if (Array.isArray(d.exclusionItems) && d.exclusionItems.length > 0) setExclusionItems(d.exclusionItems);
      if (Array.isArray(d.paymentTerms) && d.paymentTerms.length > 0) setPaymentTerms(d.paymentTerms);
      if (Array.isArray(d.items) && d.items.length > 0) setItems(d.items);
      setValidityMode(d.validityMode??DEFAULT_VALIDITY_DAYS); setCustomValidityDays(d.customValidityDays??"");
    } catch {}
  }, []);

  useEffect(() => {
    localStorage.setItem(DRAFT_KEY, JSON.stringify({to,attn,project,serviceTitle,inclusionItems,exclusionItems,paymentTerms,items,validityMode,customValidityDays}));
  });
  ...
  const validate = () => {
    ...
    if (!exclusionItems.some(i=>i.trim())) return "Please enter at least one exclusion.";
    if (!paymentTerms.some(i=>i.trim())) return "Please enter at least one payment term.";
    return "";
  };

  const buildData = (): Omit<QuotationData, "quotationNo"> => ({
    date:date.trim(), to:to.trim(), attn:attn.trim(), project:project.trim(), serviceTitle:serviceTitle.trim(),
    introParagraph, annexure1Title:DEFAULT_ANNEXURE_1, annexure2Title:DEFAULT_ANNEXURE_2, annexure3Title:DEFAULT_ANNEXURE_3,
    closingParagraph:DEFAULT_CLOSING, signatoryName:DEFAULT_SIGNATORY_NAME, signatoryDesignation:DEFAULT_SIGNATORY_DESIGNATION,
    inclusionItems:inclusionItems.filter(i=>i.trim()), exclusionItems:exclusionItems.filter(i=>i.trim()), boqItems:items,
    totalWithoutVat:+totals.totalWithoutVat.toFixed(2), vatPercent:totals.vatPercent, vatAmount:+totals.vatAmount.toFixed(2),
    totalWithVat:+totals.totalWithVat.toFixed(2), amountInWords:autoWords, paymentTerms:paymentTerms.filter(i=>i.trim()),
    validity:validityText, attachmentNames:attachments.map(f=>f.name),
  });

  const reset = () => {
    localStorage.removeItem(DRAFT_KEY);
    setQuotationNo("(auto)"); setDate(formatToday()); setTo(""); setAttn(""); setProject(""); setServiceTitle("");
    setInclusionItems([""]); setExclusionItems([""]); setPaymentTerms(DEFAULT_PAYMENT_TERMS);
    setItems([{srNo:"1",description:"",unit:"",amount:""}]);
    setValidityMode(DEFAULT_VALIDITY_DAYS); setCustomValidityDays(""); setAttachments([]);
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await apiCall<{quotation:QuotationData}>("/api/quotation",{method:"POST",body:data});
      const qData = res.quotation;
      setQuotationNo(qData.quotationNo);
      const [{pdf},{default:QDoc}] = await Promise.all([import("@react-pdf/renderer"),import("./QuotationDocument")]);
      const blob = await pdf(<QDoc quotationData={qData}/>).toBlob();
      const file = new File([blob],`${qData.quotationNo}.pdf`,{type:"application/pdf"});
      ...
      setMessage({text:`Quotation ${qData.quotationNo} saved and downloaded.`,type:"success"});
      reset();
    } catch(e:any){setMessage({text:e.message||"Failed to generate.",type:"error"});}
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const [{pdf},{default:QDoc}] = await Promise.all([import("@react-pdf/renderer"),import("./QuotationDocument")]);
      const blob = await pdf(<QDoc quotationData={qData}/>).toBlob();
      const pdfFile = new File([blob],`${qData.quotationNo}.pdf`,{type:"application/pdf"});
      const text = [`Quotation: ${qData.quotationNo}`,`Project: ${qData.project||"-"}`,`Total: AED ${qData.totalWithVat.toFixed(2)}`].join("\n");
      if(navigator.share){const files=[pdfFile,...attachments];if(navigator.canShare?.({files}))await navigator.share({title:qData.quotationNo,text,files});else await navigator.share({title:qData.quotationNo,text});}
      else await navigator.clipboard.writeText(text);
      setMessage({text:`Quotation ${qData.quotationNo} shared.`,type:"success"}); reset();
    } catch(e:any){setMessage({text:e.message||"Failed.",type:"error"});}
    finally{setIsWorking(false);}
  };

  // Styles matching the original "paper" look
  ...
  return (
    <div className="max-w-[1160px] mx-auto animate-fade-in-up">
      <div className="mb-5">
        ...
        <p className="mt-2 text-navy-400 text-sm max-w-[840px]">Quotation number is assigned automatically. Headings, annexures, exclusions, and payment terms follow the fixed PDF style.</p>
      </div>
      ...
      <div style={paper}>
        ...
        <div style={{marginTop:"34px"}}>
          ...
          <div style={{width:"120px",height:"36px",border:"1px dashed #94a3b8",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"12px",color:"#64748b",margin:"10px 0 6px"}}>quotation-signature.png</div>
          <p style={bodyP}>{DEFAULT_SIGNATORY_NAME}</p>
          <p style={bodyP}>{DEFAULT_SIGNATORY_DESIGNATION}</p>
        </div>
      </div>

      {/* Page 2: Annexure 1 + BOQ */}
      <div style={paper}>
        ...
        <div className="grid gap-2.5">
          {inclusionItems.map((item,i)=>(
            <div key={i} className="flex gap-2.5 items-start">
              <textarea value={item} onChange={e=>updateInclusion(i,e.target.value)} style={{flex:1,minHeight:"76px",padding:"10px 12px",border:"1px solid #cbd5e1",borderRadius:"6px",fontSize:"15px",fontFamily:"'Times New Roman',Georgia,serif",resize:"vertical",boxSizing:"border-box"}} />
              <button onClick={()=>removeInclusion(i)} disabled={inclusionItems.length===1} className="px-4 py-2 rounded-lg bg-red-600 text-white font-semibold text-sm hover:bg-red-700 disabled:opacity-50 transition-all">Remove</button>
            </div>
          ))}
        </div>
        <div style={{marginTop:10,marginBottom:14}}>
          <button onClick={addInclusion} className="px-4 py-2 rounded-lg bg-white text-navy border border-navy font-semibold text-sm hover:bg-navy-50 transition-all">Add Inclusion</button>
        </div>
        ...
        <div className="grid gap-2.5">
          {exclusionItems.map((item,i)=>(
            <div key={i} className="flex gap-2.5 items-start">
              <textarea value={item} onChange={e=>updateExclusion(i,e.target.value)} style={{flex:1,minHeight:"76px",padding:"10px 12px",border:"1px solid #cbd5e1",borderRadius:"6px",fontSize:"15px",fontFamily:"'Times New Roman',Georgia,serif",resize:"vertical",boxSizing:"border-box"}} />
              <button onClick={()=>removeExclusion(i)} disabled={exclusionItems.length===1} className="px-4 py-2 rounded-lg bg-red-600 text-white font-semibold text-sm hover:bg-red-700 disabled:opacity-50 transition-all">Remove</button>
            </div>
          ))}
        </div>
        <div style={{marginTop:10,marginBottom:14}}>
          <button onClick={addExclusion} className="px-4 py-2 rounded-lg bg-white text-navy border border-navy font-semibold text-sm hover:bg-navy-50 transition-all">Add Exclusion</button>
        </div>
        ...
        <div style={{border:"1px solid #111827",marginTop:"10px"}}>
          <div style={{display:"flex",background:"#d8e4f2",borderBottom:"1px solid #111827"}}>
            <div style={{padding:"8px 6px",fontSize:"15px",fontWeight:700,textAlign:"center",borderRight:"1px solid #111827",width:"12%"}}>Sr. no.</div>
            <div style={{padding:"8px 6px",fontSize:"15px",fontWeight:700,textAlign:"center",borderRight:"1px solid #111827",width:"59%"}}>Description</div>
            <div style={{padding:"8px 6px",fontSize:"15px",fontWeight:700,textAlign:"center",borderRight:"1px solid #111827",width:"11%"}}>Unit</div>
            <div style={{padding:"8px 6px",fontSize:"15px",fontWeight:700,textAlign:"center",width:"18%"}}>Amount (AED)</div>
          </div>
          {items.map((item,i)=>(
            <div key={i} style={{display:"flex",borderBottom:"1px solid #111827",minHeight:"58px"}}>
              <div style={{padding:0,borderRight:"1px solid #111827",width:"12%",display:"flex",alignItems:"center",justifyContent:"center"}}>{item.srNo}</div>
              <div style={{padding:0,borderRight:"1px solid #111827",width:"59%",display:"flex",alignItems:"stretch"}}>
                <textarea value={item.description} onChange={e=>updateItem(i,"description",e.target.value)} style={{width:"100%",minHeight:"56px",border:"none",outline:"none",resize:"vertical",padding:"8px 6px",fontSize:"15px",lineHeight:1.25,fontFamily:"'Times New Roman',Georgia,serif"}} />
              </div>
              <div style={{padding:0,borderRight:"1px solid #111827",width:"11%",display:"flex",alignItems:"stretch"}}>
                <input value={item.unit} onChange={e=>updateItem(i,"unit",e.target.value)} style={{width:"100%",border:"none",outline:"none",padding:"8px 6px",fontSize:"15px",fontFamily:"'Times New Roman',Georgia,serif"}} />
              </div>
              <div style={{padding:0,width:"18%",display:"flex",alignItems:"stretch"}}>
                <input value={item.amount} onChange={e=>updateItem(i,"amount",e.target.value)} style={{width:"100%",border:"none",outline:"none",padding:"8px 6px",fontSize:"15px",fontFamily:"'Times New Roman',Georgia,serif"}} />
              </div>
            </div>
          ))}
          <div style={{display:"flex",gap:"10px",padding:"10px 8px",borderTop:"1px solid #111827"}}>
            <button onClick={addRow} className="px-4 py-2 rounded-lg bg-white text-navy border border-navy font-semibold text-sm hover:bg-navy-50 transition-all">Add BOQ Row</button>
            <button onClick={()=>removeRow(items.length-1)} disabled={items.length===1} className="px-4 py-2 rounded-lg bg-red-600 text-white font-semibold text-sm hover:bg-red-700 disabled:opacity-50 transition-all">Remove Last</button>
          </div>
        </div>
      </div>
      ...
      <div style={ulHC}>ANNEXURE-3:</div>
        

        <div style={{marginTop:22}}>
          <div style={ulH}>PAYMENT TERMS:</div>
          {paymentTerms.map((term, i) => (
            <div key={`pt-${i}`} className="flex gap-2 items-start mb-2">
              <textarea
                value={term}
                onChange={e => updatePaymentTerm(i, e.target.value)}
                style={{flex:1,minHeight:"76px",padding:"10px 12px",border:"1px solid #cbd5e1",borderRadius:"6px",fontSize:"15px",fontFamily:"'Times New Roman',Georgia,serif",resize:"vertical",boxSizing:"border-box"}}
              />
              <button
                type="button"
                onClick={() => removePaymentTerm(i)}
                disabled={paymentTerms.length===1}
                className="px-4 py-2 rounded-lg bg-red-600 text-white font-semibold text-sm hover:bg-red-700 disabled:opacity-50 transition-all"
              >Remove</button>
            </div>
          ))}
          <button onClick={addPaymentTerm} className="px-4 py-2 rounded-lg bg-white text-navy border border-navy font-semibold text-sm hover:bg-navy-50 transition-all">Add Payment Term</button>
        </div>

        <div style={{marginTop:22}}>
          <div style={ulH}>VALIDITY:</div>
          <div className="flex gap-2.5 items-center mb-2.5">
            <select value={validityMode} onChange={e=>setValidityMode(e.target.value)} style={{minHeight:"40px",minWidth:"180px",padding:"8px 10px",fontSize:"15px",fontFamily:"'Times New Roman',Georgia,serif",border:"1px solid #cbd5e1",borderRadius:"6px",background:"#fff"}}>
              <option value="15">15 Days</option><option value="30">30 Days</option><option value="45">45 Days</option><option value="60">60 Days</option><option value="90">90 Days</option><option value="custom">Custom</option>
            </select>
        ...
        <div style={{marginTop:22}}>
          ...
          {attachments.length>0&&<div className="grid gap-2 mt-3">{attachments.map((f,i)=>(
            <div key={`${f.name}-${i}`} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"12px",padding:"10px 12px",borderRadius:"8px",background:"#f8fafc",border:"1px solid #d9deea"}}>
              <span>{f.name}</span>
              <button onClick={()=>setAttachments(p=>p.filter((_,j)=>j!==i))} className="px-3 py-1 bg-red-600 text-white rounded-lg text-xs font-semibold hover:bg-red-700">Remove</button>
            </div>
          ))}</div>}
        </div>
      </div>
      ...
      <div className="flex justify-end gap-3 flex-wrap">
        <button onClick={handleGenerate} disabled={isWorking} className="px-6 py-3 bg-navy text-white font-bold rounded-xl text-sm shadow-lg shadow-navy/20 hover:bg-navy-700 hover:-translate-y-0.5 disabled:opacity-50 transition-all">{isWorking?"Working…":"Generate PDF"}</button>
        <button onClick={handleShare} disabled={isWorking} className="px-6 py-3 bg-navy text-white font-bold rounded-xl text-sm shadow-lg shadow-navy/20 hover:bg-navy-700 disabled:opacity-50 transition-all">Share</button>
      </div>
    </div>
  );
}
...
function RowLabelInput({label,value,onChange,readOnly=false,multiline=false}:{label:string;value:string;onChange?:(v:string)=>void;readOnly?:boolean;multiline?:boolean}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:"82px 1fr",gap:"12px",alignItems:"start"}}>
    ...
    <div style={{width:"100%"}}>{multiline?<textarea value={value} readOnly={readOnly} onChange={e=>onChange?.(e.target.value)} style={base}/>:<input value={value} readOnly={readOnly} onChange={e=>onChange?.(e.target.value)} style={base}/>}</div>
  </div>);
}
...
function RowSubjectInput({label,prefix,value,onChange}:{label:string;prefix:string;value:string;onChange:(v:string)=>void}){
  return(<div style={{display:"grid",gridTemplateColumns:"82px 1fr",gap:"12px",alignItems:"start"}}>
    ...
    <div style={{display:"flex",alignItems:"center",gap:6}}>
      ...
      <input value={value} onChange={e=>onChange(e.target.value)} style={{flex:1,minHeight:"38px",padding:"8px 10px",fontSize:"16px",fontFamily:"'Times New Roman',Georgia,serif",border:"1px solid #cbd5e1",borderRadius:"6px"}}/>
    </div>
  </div>);
}
