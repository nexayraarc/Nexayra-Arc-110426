...
export default function CreateLPO() {
  ...
  useEffect(() => {
    try {
      ...
      setClientName(d.clientName ?? ""); setClientPhone(d.clientPhone ?? ""); setProject(d.project ?? "");
      setSiteLocation(d.siteLocation ?? ""); setContact(d.contact ?? ""); setReference(d.reference ?? "");
      setVendorName(d.vendorName ?? ""); setVendorAddress(d.vendorAddress ?? ""); setVendorPhone(d.vendorPhone ?? "");
      setVendorTRN(d.vendorTRN ?? ""); setPaymentPreset(d.paymentPreset ?? "30"); setCustomPaymentDays(d.customPaymentDays ?? "");
      setDeliveryNumber(d.deliveryNumber ?? ""); setDeliveryPreset(d.deliveryPreset ?? "Days");
      setCustomDeliveryTime(d.customDeliveryTime ?? ""); setRequestedBy(d.requestedBy ?? ""); setPreparedBy(d.preparedBy ?? "");
  ...
  // Draft save
  useEffect(() => {
    localStorage.setItem(DRAFT_KEY, JSON.stringify({ clientName, clientPhone, project, siteLocation, contact, reference, vendorName, vendorAddress, vendorPhone, vendorTRN, paymentPreset, customPaymentDays, deliveryNumber, deliveryPreset, customDeliveryTime, requestedBy, preparedBy, items }));
  });
  ...
  const resolvedDelivery = useMemo(() => {
    if (deliveryPreset === "custom") { const ct = customDeliveryTime.trim(); return { dp: "", dt: deliveryNumber.trim() && ct ? `${deliveryNumber.trim()} ${ct}` : ct }; }
  ...
  const calcItem = (item: LpoItem) => {
    const qty = Number(item.qty || 0), up = Number(item.amount || 0), dp = Math.max(0, Number(item.discount || 0));
    const g = qty * up, da = g * (dp / 100), st = g - da, vt = st * 0.05;
  ...
  const validate = () => {
    ...
    if (!project.trim()) return "Please enter Project Name.";
  ...
  const clearDraft = () => {
    ...
    setNxrNo(null); setClientName(""); setClientPhone(""); setProject(""); setSiteLocation(""); setContact(""); setReference("");
    setVendorName(""); setVendorAddress(""); setVendorPhone(""); setVendorTRN(""); setPaymentPreset("30"); setCustomPaymentDays("");
    setDeliveryNumber(""); setDeliveryPreset("Days"); setCustomDeliveryTime(""); setRequestedBy(""); setPreparedBy("");
    setItems([makeEmpty()]); setAttachments([]);
  };

  const generatePdf = async (data: LpoPdfData) => {
    const [{ pdf }, { default: LpoDocument }] = await Promise.all([import("@react-pdf/renderer"), import("./LpoDocument")]);
    const blob = await pdf(<LpoDocument lpoData={data} />).toBlob();
    return new File([blob], `LPO-${data.nxrNo}.pdf`, { type: "application/pdf" });
  };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const payload = {
        clientName: clientName.trim(), clientPhone: clientPhone.trim(), project: project.trim(),
        siteLocation: siteLocation.trim(), contact: contact.trim(), reference: reference.trim(),
        vendorName: vendorName.trim(), vendorAddress: vendorAddress.trim(), vendorPhone: vendorPhone.trim(), vendorTRN: vendorTRN.trim(),
        items, totalDiscount: +totals.discountAmount.toFixed(2), subtotal: +totals.subtotal.toFixed(2), vat: +totals.vat.toFixed(2), total: +totals.total.toFixed(2),
      ...
      const res = await apiCall<{ lpo: LpoPdfData }>("/api/lpo", { method: "POST", body: payload });
      setNxrNo(res.lpo.nxrNo);
      const pdfFile = await generatePdf(res.lpo);
      const url = URL.createObjectURL(pdfFile);
      Object.assign(document.createElement("a"), { href: url, download: pdfFile.name }).click();
      URL.revokeObjectURL(url);
      setMessage({ text: `LPO #${res.lpo.nxrNo} saved and downloaded.`, type: "success" });
      clearDraft();
    } catch (e: any) {
      setMessage({ text: e.message || "Failed to generate LPO.", type: "error" });
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const res = await apiCall<{ lpo: LpoPdfData }>("/api/lpo", { method: "POST", body: payload });
      const pdfFile = await generatePdf(res.lpo);
      const text = [`LPO #${res.lpo.nxrNo}`, `Client: ${res.lpo.clientName}`, `Project: ${res.lpo.project}`, `Total: AED ${res.lpo.total.toFixed(2)}`].join("\n");
      if (navigator.share) {
      ...
      setMessage({ text: `LPO #${res.lpo.nxrNo} shared.`, type: "success" });
      clearDraft();
    } catch (e: any) {
      setMessage({ text: e.message || "Failed to share.", type: "error" });
    } finally { setIsWorking(false); }
  };
    ...
    <div className="max-w-5xl mx-auto animate-fade-in-up">
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        ...
        <div className="flex gap-3">
          <button onClick={handleDownload} disabled={isWorking} className="px-5 py-2.5 bg-navy hover:bg-navy-700 text-white font-bold rounded-xl text-sm disabled:opacity-50 transition-all shadow-lg shadow-navy/20 hover:shadow-xl hover:-translate-y-0.5">{isWorking ? "Working…" : "Generate PDF"}</button>
          <button onClick={handleShare} disabled={isWorking} className="px-5 py-2.5 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl text-sm disabled:opacity-50 transition-all shadow-lg shadow-teal-600/20">{isWorking ? "Working…" : "Share"}</button>
        </div>
      </div>

      {message && (
        <div className={`mb-5 p-3.5 rounded-xl text-sm font-medium animate-scale-in ${message.type === "success" ? "bg-green-50 border border-green-200 text-green-700" : "bg-red-50 border border-red-200 text-red-600"}`}>{message.text}</div>
      )}

      <div className="bg-white border border-navy-100 rounded-2xl p-6 sm:p-8 shadow-sm">
        <h3 className={sec}>Project Details</h3>
        <div className="grid sm:grid-cols-2 gap-4">
          <div><label className={lbl}>Project Code</label><input placeholder="Project Code" value={project} onChange={e => setProject(e.target.value)} className={inp} /></div>
          <div><label className={lbl}>Project Name</label><input placeholder="Project Name" value={clientName} onChange={e => setClientName(e.target.value)} className={inp} /></div>
          <div><label className={lbl}>Contact Person</label><input placeholder="Contact Person" value={contact} onChange={e => setContact(e.target.value)} className={inp} /></div>
          <div><label className={lbl}>Contact Number</label><input placeholder="Contact Number" value={clientPhone} onChange={e => setClientPhone(e.target.value)} className={inp} /></div>
          <div><label className={lbl}>Site Location</label><input placeholder="Site Location" value={siteLocation} onChange={e => setSiteLocation(e.target.value)} className={inp} /></div>
          <div><label className={lbl}>Quotation Ref.</label><input placeholder="Reference Number" value={reference} onChange={e => setReference(e.target.value)} className={inp} /></div>
        </div>
        ...
        {items.map((item, i) => (
          <div key={i} className="flex flex-wrap md:flex-nowrap gap-2 mb-3 items-center animate-fade-in">
            ...
            <button onClick={() => setItems(p => p.length === 1 ? p : p.filter((_, j) => j !== i))} disabled={items.length === 1} className="w-20 py-3 bg-red-50 hover:bg-red-100 text-red-500 font-semibold rounded-xl text-xs disabled:opacity-30 transition-all border border-red-200">Remove</button>
          </div>
        ))}
        <button onClick={() => setItems(p => [...p, makeEmpty()])} className="mt-2 px-4 py-2.5 bg-navy-50 border border-navy-200 rounded-xl text-navy text-sm font-semibold hover:bg-navy-100 transition-all">+ Add Item</button>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {deliveryPreset === "custom" && <input placeholder="Custom delivery term" value={customDeliveryTime} onChange={e => setCustomDeliveryTime(e.target.value)} className={`${inp} w-auto min-w-[200px]`} />}
        </div>
        ...
        {attachments.length > 0 && (
          <div className="mt-3 space-y-2">
            {attachments.map((f, i) => (
              <div key={`${f.name}-${i}`} className="flex items-center justify-between gap-3 px-4 py-2.5 bg-navy-50 border border-navy-100 rounded-xl animate-fade-in">
                <span className="text-navy text-sm truncate">{f.name}</span>
                <button onClick={() => setAttachments(p => p.filter((_, j) => j !== i))} className="text-red-500 text-xs font-semibold hover:text-red-700">Remove</button>
              </div>
            ))}
          </div>
        )}
